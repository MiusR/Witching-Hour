...
execute as @e[type=armor_stand,tag=soullink,tag=!init] at @s positioned ~ ~ ~ store result score @s Taglocktemp run scoreboard players get @p[dy=6,dx=0.4,dz=0.4] Taglock
...
execute as @e[tag=soullinked,tag=!init] at @s positioned ~ ~ ~ run kill @e[type=armor_stand,tag=soullink,tag=init,dy=6,dx=0.4,dz=0.4,limit=1]
...
execute as @e[tag=bounddisplay] at @s positioned ~ ~ ~ if entity @e[tag=soullinked,dy=6,dx=0.4,dz=0.4] run kill @s
