...
execute if entity @a[tag=inSoulDim] as @e[tag=!inSoulDim,tag=!summonSoul,tag=!soulDimSoul,type=!item,type=!zombie_pigman,type=!armor_stand,y=130,dy=36] at @s run tag @s add remove
execute as @e[tag=addremove] at @s run tp ~ 0 ~
...
execute as @e[tag=summonSoul] at @s run tp @s ~ 0 ~
