execute as @e[type=armor_stand,tag=cauldron,tag=!timer] at @s if entity @e[type=item,nbt={Item:{tag:{"id":"moss_crown_incomplete"}}},distance=..1] if entity @e[type=item,nbt={Item:{id:"minecraft:nether_star"}},distance=..1] if block ~ ~ ~ cauldron[level=3] if entity @e[tag=magicaltar,distance=..20,scores={Power=100..},limit=1] if block ~ ~-1 ~ fire run tag @s add rmcf
execute as @e[type=armor_stand,tag=cauldron,tag=!timer,tag=rmcf] at @s run tag @s add timer
execute as @e[type=armor_stand,tag=cauldron,tag=rmcf] at @s run scoreboard players set @e[tag=magicaltar,distance=..20,scores={Power=100..},limit=1] mb_PowerCost 100
