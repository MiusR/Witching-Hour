...
execute as @a[nbt={RootVehicle:{Entity:{Tags:["flying_broom"]}}},scores={Sneak=1..}] at @s run kill @e[tag=broom_saddle,sort=nearest,limit=1]
execute as @e[tag=flying_broom] at @s unless entity @e[tag=broom_saddle,distance=..1] run kill @e[type=item,sort=nearest,limit=1,nbt={Item:{id:"minecraft:saddle"}}]
